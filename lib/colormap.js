...
/** Wind speed ramp (km/h). Same rationale as TEMP_STOPS: our own choice. */
const WIND_STOPS = [
  [0,  '#eaf5ff'],
  [10, '#a8d5f0'],
  [20, '#4a9fd8'],
...
/** Wind speed (km/h) -> [r,g,b]. */
function windRgb(v) {
  if (v == null || !Number.isFinite(v)) return hexToRgb(SEA);
  if (v <= WIND_STOPS[0][0]) return hexToRgb(WIND_STOPS[0][1]);
  for (let i = 1; i < WIND_STOPS.length; i++) {
    if (v <= WIND_STOPS[i][0]) {
      const [v0, c0] = WIND_STOPS[i - 1];
      const [v1, c1] = WIND_STOPS[i];
      return mix(c0, c1, (v - v0) / (v1 - v0 || 1));
    }
  }
  return hexToRgb(WIND_STOPS[WIND_STOPS.length - 1][1]);
}

/** Static ramp stops, for handing to a client that wants to colour its own. */
function rampStops() {
  return { temperature: TEMP_STOPS, wind: WIND_STOPS, sea: SEA };
}
...
module.exports = { TEMP_STOPS, WIND_STOPS, SEA, tempRgb, windRgb, renderFieldRGBA, hexToRgb, mix, rampStops };
